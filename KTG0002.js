...
export class KTG0002 extends KanbanGridBase {
    ...
    ContainerSetup() {
        ...
        let html = `<h2 style="color:${this.titlecolor || 'black'};text-align: center;">${this.title || 'SANTONI织机车间进度看板   TIẾN ĐỘ SẢN LƯỢNG DỆT SANTONI ' + this.workshop || "" }</h2>
    ...
    GridSetup() {
        ...
        this.grid.option('columns', [
            ...
            { "dataField": "WorkLineIndex", "caption": `机台号\nSố máy`, "dataType": "number", "headerCellTemplate": brheader,"width":0.0000001 },
            { "dataField": "StyleNo", "caption": `款号\nMã hàng`, "dataType": "string", "headerCellTemplate": brheader },
                ...
                if(e.name == "Quantity") {
                    ...
                    switch(e.summaryProcess) {
                        ...
                        case "finalize":
                            ...
                            // Assigning the final value to "totalValue" here
                            break;
                            // Tỉ lệ hoàn thành của total + group
                    }
                }
            } 
        });
        this.grid.option('onCellPrepared', (e) => {
            if (e.rowType == "header") {
                ...
                if (e.data.WorkLineIndex != 1) temp.first.push(e.rowIndex);
                if (typeof e.value === 'string' && e.columnIndex === 7) {

                    if (parseInt(e.value.slice(0, e.value.length)) < 100 && this.withOutTime(new Date(e.data.ExportDate)) - this.withOutTime(Date.now()) < 864000000) e.cellElement.css('background-color', 'red');  
                    // COLOR RED Ngày xuất hàng cách thời điểm hiện tại nhỏ hơn 10 ngày  và tỉ lệ hoàn thành nhỏ hơn 100%
                }
    ...
    LoadData() {
        ...
        AjaxBase(null, this.url || "/STN/STN0001", "POST", { Workshop: this.workshop }, (response) => {
